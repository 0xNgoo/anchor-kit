...
describe('MVP Express-mounted integration', () => {
  ...
  it('2e) deposit interactive returns server_misconfigured when interactiveDomain not set', async () => {
    const customDbUrl = makeSqliteDbUrlForTests();
    const customAnchor = createAnchor({
      network: { network: 'testnet' },
      server: { port: 3002 /* different port for safety */ },
      security: {
        sep10SigningKey: sep10ServerKeypair.secret(),
    ...
    expect(response.body.error).toBe('server_misconfigured');

    await customAnchor.shutdown();
    const customDbPath2 = customDbUrl.startsWith('file:') ? customDbUrl.slice('file:'.length) : customDbUrl;
    try {
      unlinkSync(customDbPath2);
    } catch {
      // ignore cleanup
    }
  });

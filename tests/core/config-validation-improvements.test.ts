...
describe('Config Validation Improvements (#124, #125)', () => {
  ...
  it('should reject non-HTTP(S) schemes for server.interactiveDomain', () => {
    const badConfig: AnchorKitConfig = {
      ...validBaseConfig,
      server: { interactiveDomain: 'javascript:alert(1)' as unknown as string },
    };
    const config = new AnchorConfig(badConfig);
    expect(() => config.validate()).toThrow(ConfigError);
    expect(() => config.validate()).toThrow(/Invalid URL format for server.interactiveDomain/);
  });

  it('should reject non-HTTP(S) schemes for metadata.tomlUrl', () => {
    const badConfig: AnchorKitConfig = {
      ...validBaseConfig,
      metadata: { tomlUrl: 'file:///etc/passwd' } as unknown as AnchorKitConfig['metadata'],
    };
    const config = new AnchorConfig(badConfig);
    expect(() => config.validate()).toThrow(ConfigError);
    expect(() => config.validate()).toThrow(/Invalid URL format for metadata.tomlUrl/);
  });
});

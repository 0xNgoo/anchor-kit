...
export class AnchorExpressRouter {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (method === 'POST' && path === '/transactions/deposit/interactive') {
      ...
      // interactiveDomain must be configured for interactive flows
    ...
    if (method === 'GET' && transactionMatch) {
      ...
      const responseData: Record<string, unknown> & { more_info_url?: string } = {
        ...
        // interactive_url is only returned when an interactiveDomain is configured
        ...(serverConfig.interactiveDomain
          ? { interactive_url: `${serverConfig.interactiveDomain}/deposit/${transaction.id}` }
          : {}),
